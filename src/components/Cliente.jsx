...
import { deleteCustomer } from '../data/clientes'

export async function action({params}) {
    await deleteCustomer(params.clienteId)
    return redirect('/')
}
...
const Cliente = ({cliente}) => {
    ...
    return(
        <tr className="border-b">
            ...
            <td>
                <p className="text-gray-600"><span className="text-gray-800 uppercase font-bold"></span>{notas}</p>
            </td>
            <td className="p-6 flex gap-3">
                ...
                onSubmit={(e) => {
                    if(!confirm("¿Esta seguro de querer eliminar el registro seleccionado?")) {
                        e.preventDefault()
                    }
                }}
                >
